...
**Следствие:** Все хуки должны запускаться **через WSL**, не нативно в Windows. Пути должны быть linux-style (`<projects-root>/...`), uv и python должны быть доступны в WSL-среде.
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "uv run --directory <projects-root>/memory\\ claude/memory\\ claude python hooks/codex/session-start.py",
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "uv run --directory <projects-root>/memory\\ claude/memory\\ claude python hooks/codex/stop.py",
            "timeout": 10
          }
        ]
      }
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "uv run --directory <projects-root>/memory\\ claude/memory\\ claude python hooks/codex/user-prompt-wiki.py",
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "uv run --directory <projects-root>/memory\\ claude/memory\\ claude python hooks/codex/post-tool-capture.py",
            "timeout": 3
          }
        ]
      }
...
```bash
echo '{"session_id":"test-001","cwd":"<projects-root>/game app/messenger_test","transcript_path":"/tmp/test.jsonl","hook_event_name":"SessionStart","model":"o4-mini","permission_mode":"suggest","source":"startup"}' | \
  uv run --directory <repo-root> python hooks/codex/session-start.py
# Ожидаем: JSON с additionalContext, секция Project: messenger
```

### Тест 3: UserPromptSubmit (полный payload)
```bash
echo '{"session_id":"test-001","cwd":"<projects-root>","transcript_path":"/tmp/test.jsonl","hook_event_name":"UserPromptSubmit","model":"o4-mini","permission_mode":"suggest","prompt":"как работает pgvector?","turn_id":"turn-1"}' | \
  uv run --directory <repo-root> python hooks/codex/user-prompt-wiki.py
# Ожидаем: JSON с additionalContext содержащим pgvector статью
```

### Тест 4: PostToolUse (полный payload)
```bash
echo '{"session_id":"test-001","cwd":"<projects-root>/office","transcript_path":"/tmp/test.jsonl","hook_event_name":"PostToolUse","model":"o4-mini","permission_mode":"suggest","tool_name":"Bash","tool_input":{"command":"git commit -m fix"},"tool_response":"[master abc123] fix","tool_use_id":"call-001","turn_id":"turn-2"}' | \
  uv run --directory <repo-root> python hooks/codex/post-tool-capture.py
