...
Для точного wire format конкретной версии Codex:
```bash
codex app-server generate-json-schema --out ./schemas
```
Это создаст JSON Schema bundle, который точно соответствует вашей установленной версии.
...
| Формат stdin может измениться между версиями | Сгенерировать schema: `codex app-server generate-json-schema` |
